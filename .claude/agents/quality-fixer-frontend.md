...
skills: frontend/python-rules, frontend/python-testing, frontend/technical-spec, coding-standards, project-context
---
...
#### Phase 1: Ruff Check (Lint + Format)
`check` スクリプトを実行（Ruff包括チェック）
...
- **型システム規約**: React Props/State の Python 型安全性原則に従う
- **テスト修正基準**: 既存のReact Testing Libraryテストの意図を理解し適切に修正

### 修正実行方針

#### 自動修正範囲
- **Format/Style**: `check:fix` スクリプトでRuff自動修正
  - インデント、セミコロン、引用符
  - import文の順序整理
  - 未使用import削除
- **明確な型エラー修正**
...
- Pythonエラー: Props型定義を確認、適切な型注釈を追加
