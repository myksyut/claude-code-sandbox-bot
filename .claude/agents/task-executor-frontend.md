...
skills: frontend/python-rules, frontend/python-testing, coding-standards, project-context, frontend/technical-spec, implementation-approach
---
...
### パッケージマネージャー確認
pyproject.tomlの`packageManager`フィールドに応じた実行コマンドを使用すること。

### 実装への反映
- アーキテクチャルールでコンポーネント階層・データフローを決定
- Pythonルールで型定義（React Props、State）・エラーハンドリングを実装
...
```json
{
  ...
  "filesModified": ["src/components/Button/Button.py", "src/components/Button/index.ts"],
  "testsAdded": ["src/components/Button/Button.test.py"],
  "newTestsPassed": true,
  "progressUpdated": {
    "taskFile": "5/8 項目完了",
...
```json
{
  ...
  "similar_components": [
    {
      "file_path": "src/components/ExistingButton/ExistingButton.py",
